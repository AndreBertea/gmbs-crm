-- Fonction SQL pour récupérer les détails d'une intervention par son ID externe
-- Usage: SELECT * FROM get_intervention_by_external_id('4281');

CREATE OR REPLACE FUNCTION get_intervention_by_external_id(external_id text)
RETURNS TABLE (
...
BEGIN
    ...
    WITH intervention_data AS (
        ...
        WHERE i.id_inter = external_id AND i.is_active = true
    ),
    costs_summary AS (
        SELECT 
            intervention_id,
...
CREATE OR REPLACE FUNCTION get_intervention_summary_by_external_id(external_id text)
RETURNS TABLE (
    artisan_plain_nom text,
    cout_intervention numeric,
    statut_nom text,
...
-- Exemple 1: Récupérer tous les détails d'une intervention par ID externe
-- SELECT * FROM get_intervention_by_external_id('4281');

-- Exemple 2: Récupérer juste le résumé d'une intervention par ID externe
-- SELECT * FROM get_intervention_summary_by_external_id('4281');
...
-- FROM get_intervention_summary_by_external_id('4281');

-- Exemple 4: Test avec une valeur fixe
SELECT * FROM get_intervention_summary_by_external_id('4281');
