...
/**
 * Prépare les documents pour l'insertion (tous avec kind = "a classifier")
 */
function prepareDocumentsForInsertion(documents) {
  return documents.map(doc => ({
    ...
    kind: 'a classifier' // Tous les documents sont classifiés comme "a classifier"
  }));
}
...
`);
  ...
  console.log('📌 Note: Tous les documents seront classifiés avec kind = "a classifier"\n');
    ...
    for (let i = 0; i < foldersWithId.length; i++) {
      ...
      if (matchResult.intervention && !skipInsert && drive && folder.folderId) {
        ...
        if (driveDocuments.length > 0) {
          ...
          console.log(`       - Tous classifiés comme: "a classifier"`);
