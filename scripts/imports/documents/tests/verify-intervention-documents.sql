...
SELECT 
  ...
  COUNT(*) FILTER (WHERE kind = 'a_classe') as documents_a_classer,
...
-- 3. Documents "à classer" par intervention
SELECT 
  i.id_inter,
  COUNT(*) as nb_documents_a_classer,
  STRING_AGG(ia.filename, ', ' ORDER BY ia.filename) as fichiers
FROM public.intervention_attachments ia
JOIN public.interventions i ON ia.intervention_id = i.id
WHERE ia.kind = 'a_classe'
GROUP BY i.id_inter
ORDER BY nb_documents_a_classer DESC
LIMIT 20;
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE ia.kind = 'a_classe') as nb_a_classer,
...
-- 12. Top 10 des interventions avec le plus de documents "à classer"
SELECT 
  i.id_inter,
  COUNT(*) as nb_a_classer,
  STRING_AGG(ia.filename, ', ' ORDER BY ia.filename LIMIT 5) as exemples_fichiers
FROM public.intervention_attachments ia
JOIN public.interventions i ON ia.intervention_id = i.id
WHERE ia.kind = 'a_classe'
GROUP BY i.id_inter
ORDER BY nb_a_classer DESC
LIMIT 10;
